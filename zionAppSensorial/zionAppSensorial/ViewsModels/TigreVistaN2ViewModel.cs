...
{
    public class TigreVistaN2ViewModel : BindableObject
    {
        ...
        public ICommand TigreButtonClickedCommand { get; }
        ...
        public TigreVistaN2ViewModel()
        {
            IncorrectButtonCommand = new Command<string>(async (button) => await OnIncorrectButtonClicked(button));
            TigreButtonClickedCommand = new Command(async () => await OnTigreButtonClicked());
        ...
        private async Task OnTigreButtonClicked()
        {
            _hasSelectedCorrect = true;
            TigreButtonColor = Color.Green;
        ...
        {
            var text = "Con rayas en mi piel y un rugido feroz, en la selva domino, ¿qué soy?";
